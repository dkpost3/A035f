...
struct sprd_hsphy {
	...
	u32			tfregres_value;
	/* Tab A8 code for AX6300DEV-34 by wangjian at 20210810 end */
};

#define TUNEHSAMP_2_6MA		(3 << 25)
/* Tab A8 code for AX6300DEV-34 by wangjian at 20210813 start */
#define TFREGRES_TUNE_VALUE	(0x0C << 19)
...
static int sprd_hsphy_probe(struct platform_device *pdev)
{
	...
	/* Tab A8 code for AX6300DEV-34 by wangjian at 20210813 start */
	ret = of_property_read_u32(dev->of_node, "sprd,tfregres-tune-value",
				   &phy->tfregres_value);
	if (ret < 0) {
		dev_err(dev, "unable to read tfregres-tune-value");
		phy->tfregres_value = TFREGRES_TUNE_VALUE;
	}
	/* Tab A8 code for AX6300DEV-34 by wangjian at 20210813 end */
